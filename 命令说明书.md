...
- [x]  复读机
## 命令说明
所有命令，如无特殊说明的，则在群聊和私聊中均可生效。命令说明书中的“|”代表或，及输入多个命令中的一个或者多个参数中的一个即可。[]方括号内文字代表当前命令支持的参数列表。所有命令和参数之间均无空格。其中关于原神的【深渊信息查询】、【实时便签查询】、【玩家信息卡片查询】、【旅行札记查询】需要提前绑定uid和cookie。
## 原神相关功能具体命令
### 命令：help | 菜单 | 帮助
输入此命令，机器人将回复功能菜单。  
...
### 命令：抽卡分析[url] 
输入此命令，参数为抽卡记录页的url，机器人将回复玩家抽卡记录的分析情况。至于抽卡记录也的url如何获取，可以百度一下。
...
## 被动技能
### 自动推送订阅账号的B站动态到群里
此命令无输入，仅支持群聊。添加订阅账号以后，当账号发送动态时，机器人将自动推送动态到微信聊天，推送内容包括文字，图片，不包括视频，视频将推送视频链接。让你不错过任何一条官方消息。  
